...
```python
from imc22 import Simulator, NeuralReflex
import numpy as np
...
![Platform](https://img.shields.io/badge/platform-RISC--V%20%7C%20FPGA-lightgrey.svg)

Hive-Reflex is an advanced modular spinal reflex controller for robotics, powered by **Digital CIM (Computing-in-Memory)** technology. It mimics biological spinal reflexes by running lightweight neural networks locally on each joint node, enabling ultra-low latency response and compliant control.

## 🌟 Key Features

- **🧠 Digital CIM Core**: Custom FPGA implementation of MAC arrays and SRAM for in-memory computing acceleration.
- **⚡ Ultra-Low Latency**: < 2 μs inference latency (Hardware validated) @ 100MHz.
- **🔄 Hybrid Control**: Dynamic superposition of PID and Neural Reflex control (variable compliance).
- **🔧 RISC-V Powered**: Integrated with Rocket Chip (RV32IMAC) for flexible system management.
- **🔋 Energy Efficient**: Dedicated RBB (Reverse Body Bias) power management for μW-level standby power.
- **🛠️ Complete Toolchain**: From ONNX models to CIM machine code via custom MLIR compiler.

## 🗂️ Project Structure

```
hive-reflex/
├── imc22_sdk/        # SDK Drivers and Firmware
├── mlir_compiler/    # MLIR Compiler Toolchain
├── examples/         # Example Applications
├── rtl/              # FPGA RTL Source Code (Verilog)
├── fpga/             # FPGA Validation Scripts and Projects
├── docs/             # Documentation
...
- Python 3.8+
- RISC-V Toolchain (GCC 12+)
- Verilator (for simulation)
- Xilinx Vivado (optional, for FPGA synthesis)

See [SETUP_GUIDE.md](fpga/SETUP_GUIDE.md) for detailed installation instructions.

### 2. Run Simulation

```bash
# Verify the CIM Core hardware
cd sim
./run_sim.ps1  # Windows
# ./run_sim.sh # Linux
```
...
```bash
# Compile an ONNX model to CIM instructions
cd mlir_compiler
./build.sh --model mobilenet_v1.onnx --quantize int8
```

## 📖 Documentation

- **Hardware Architecture**: [hive_arch.md](docs/hive_arch.md)
- **FPGA Validation**: [WEEK1_FINAL_REPORT.md](fpga/WEEK1_FINAL_REPORT.md)
- **SDK Guide**: [imc22_sdk/README.md](imc22_sdk/README.md)
- **MLIR Guide**: [mlir_optimization_guide.md](docs/mlir_optimization_guide.md)
